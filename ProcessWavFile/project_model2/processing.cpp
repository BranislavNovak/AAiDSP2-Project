...
extern const DSPfract initial_input_gain = 0.501187;			// (-6dB)
extern const DSPfract initial_headroom_gain = 0.707946;			// (-3dB)
...
void processing()
{
	...
	for (ptr = sampleBuffer[0]; ptr < sampleBuffer[0] + BLOCK_SIZE; ptr++)
	{
		...
		*ptr_tmp_left = FRACT_NUM(*ptr);															// Saving state to LEFT tmp buffer
		ptr_tmp_left++;

		*(ptr + BLOCK_SIZE) = *(ptr + BLOCK_SIZE) * initial_input_gain;								// R*(-6dB)
		*ptr_tmp_right = FRACT_NUM(*(ptr + BLOCK_SIZE));											// Saving state to RIGHT tmp buffer
		ptr_tmp_right++;
	}
	...
	for (ptr = sampleBuffer[0]; ptr < sampleBuffer[0] + BLOCK_SIZE; ptr++)
	{
		...
		*ptr = *((ptr + (2 * BLOCK_SIZE))) * FRACT_NUM(MINUS_SIX_DB);								// L = C*(-6dB)
		*(ptr + BLOCK_SIZE) = *((ptr + (2 * BLOCK_SIZE))) * FRACT_NUM(MINUS_SIX_DB);				// R = C*(-6dB)

																									// Passing left surrond to original Ls buffer with -2dB gain plus left channel
		*((ptr + (3 * BLOCK_SIZE))) = ((*ptr_tmp_left) * FRACT_NUM(MINUS_TWO_DB)) + *ptr;
		ptr_tmp_left++;

		// Passing right surrond to original Rs buffer with -2dB gain plus right channel
		*((ptr + (4 * BLOCK_SIZE))) = ((*ptr_tmp_right) * FRACT_NUM(MINUS_TWO_DB)) + *(ptr + BLOCK_SIZE);
		ptr_tmp_right++;
	}
}